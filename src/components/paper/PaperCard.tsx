...
import { Calendar, FileText, ChevronDown, ChevronUp, ExternalLink, Save, MessageSquareText } from 'lucide-react';
...
export const PaperCard: React.FC<PaperCardProps> = ({ paper, onChat }) => {
    ...
    return (
        <div className="group relative bg-white dark:bg-slate-900 rounded-2xl shadow-sm hover:shadow-xl hover:shadow-blue-500/10 transition-all duration-300 border border-gray-100 dark:border-slate-800">
            <div className="p-6">
                <div className="flex justify-between items-start mb-3">
                    <div className="flex-1 pr-4">
                        <div className="flex items-center gap-2 mb-2">
                            <span className="px-2 py-0.5 text-[10px] font-bold text-blue-600 bg-blue-50 dark:bg-blue-900/30 dark:text-blue-300 rounded uppercase border border-blue-100 dark:border-blue-900">
                                {paper.category}
                            </span>
                            {paper.is_downloaded && (
                                <span className="px-2 py-0.5 text-[10px] font-bold text-green-600 bg-green-50 dark:bg-green-900/30 dark:text-green-300 rounded uppercase flex items-center gap-1">
                ...
                <div 
                    className={`text-gray-600 dark:text-gray-300 text-sm leading-relaxed overflow-hidden transition-all duration-300 ${isExpanded ? 'max-h-[1000px] opacity-100 mb-4' : 'max-h-0 opacity-0'}`}
                >
                    <div className="bg-gray-50 dark:bg-slate-800/50 p-4 rounded-xl border border-gray-100 dark:border-slate-700/50">
                        {/* Dùng hàm formatAbstract ở đây */}
